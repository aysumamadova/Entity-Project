...
namespace Entity_Project
{
    class Program
    {
        ...
        {
            Console.WriteLine($"Reqem daxil edin\n1.Kitab yaradin\n2.Idye gore kitabi getirmek\n3.Kitab almaq\n4.Satilan sonuncu kitabin melumatlari\n5.Kitab silme\n6.Kitablarin melumatlari");
        ...
        static void GetAllBook()
        {
            using (AppDbContext db = new AppDbContext())
            {
                ...
                {
                    Console.WriteLine($"Id:{book.Id} Name:{book.Name} GenreName:{book.GenreName} AuthorName:{book.AuthorName} PageCount:{book.PageCount} Publishing:{book.Publishing} StockCount:{book.StockCount} Price:{book.Price}");
                }
            }
        }
        ...
        public static async Task Purchase()
        {
            using (AppDbContext db = new AppDbContext())
            {
                ...
                Console.WriteLine($"Id{result.Id} Name:{result.Name} GenreName:{result.GenreName} AuthorName:{result.AuthorName}  Price:{result.Price}");
            }
        }
